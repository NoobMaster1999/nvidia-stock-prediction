...
app.config['SECRET_KEY'] = os.getenv(
    'FLASK_SECRET_KEY', 'default_secret_key_change_in_production')
...
def validate_request():
    ...
    if request.endpoint and request.method == 'POST':
        ...
        if content_type and 'application/json' in content_type:
            try:
                if request.content_length and request.content_length > app.config[
                        'MAX_CONTENT_LENGTH']:
                    abort(413)
...
if __name__ == '__main__':
    ...
    if debug_mode:
        logger.warning(
            "Running in DEBUG mode - not recommended for production!")
